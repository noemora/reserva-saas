...
export async function createBooking(bookingData: BookingInsert) {
  try {
    ...
    const { data, error } = await supabaseServer.from("bookings").insert(bookingData).select().single()
  ...
  try {
    const { data, error } = await supabaseServer
      ...
      .select(`
        *,
        clients(name, email, phone),
        professionals!inner(name, title, company_id),
        services(name, description),
        workplaces(name, address)
      `)
...
export async function deleteBooking(bookingId: string) {
  try {
    const { error } = await supabaseServer.from("bookings").delete().eq("id", bookingId)
...
export async function getBookingsByDateRange(startDate: string, endDate: string, professionalId?: string) {
  try {
