...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      signIn: async (email, password) => {
        ...
        try {
          ...
          set({ loading: false, isLoading: false })
          return { error: { message: "No se pudo obtener la sesión" } as AuthError }
        } catch (error) {
          console.error("❌ Error inesperado en signIn:", error)
          set({ loading: false, isLoading: false })
          return { error: { message: "Error interno del servidor" } as AuthError }
        }
      },
    ...
    {
      name: "auth-storage",
      partialize: ({ session, profile, user }) => ({ session, profile, user }),
    },
  ),
)
