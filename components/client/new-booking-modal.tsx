...
import { ArrowLeft, Calendar, MapPin, User, Clock, CheckCircle } from "lucide-react"
...
export function NewBookingModal({ open, onClose }: NewBookingModalProps) {
  ...
  const handleLocationSelect = (location: BookingData["location"]) => {
    ...
    const availableProfessionals = BookingDataService.getAvailableProfessionalsForService(bookingData.service, location)
  ...
  const handleProfessionalSelect = (professional: BookingData["professional"]) => {
    ...
    switch (currentStep) {
      ...
      case "datetime":
        ...
        if (bookingData.service && bookingData.location) {
          const availableProfessionals = BookingDataService.getAvailableProfessionalsForService(
            bookingData.service,
            bookingData.location,
          )
          setCurrentStep(availableProfessionals.length > 1 ? "professional" : "location")
        } else {
          setCurrentStep("location")
        }
        break
    ...
    switch (currentStep) {
      ...
      case "location":
        return bookingData.service ? (
          <LocationSelector service={bookingData.service} onLocationSelect={handleLocationSelect} onBack={handleBack} />
        ) : null
      ...
      <div className="mb-8">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm font-medium text-blue-600">{Math.round(getProgress())}% completado</span>
          </div>
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-5xl max-h-[95vh] overflow-hidden flex flex-col">
        <DialogHeader className="pb-4 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-2xl font-bold text-gray-900">Nueva Reserva</DialogTitle>
              <p className="text-gray-600 mt-1">
                Reserva un servicio con nuestros profesionales de forma rápida y sencilla
              </p>
            </div>
